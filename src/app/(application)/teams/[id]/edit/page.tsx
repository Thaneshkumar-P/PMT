...
import { createTeam, getTeam, getTeams, updateTeam } from "../../actions";
...
export default function Page() {
  ...
  useEffect(() => {
    (async function() { 
      ...
      setSelectedUsers([...teamData.team.users])
  ...
  async function editTeam() {
    setDisabled(true)
    let res = await updateTeam(teamName, selectedUsers, id)
  
    if (res?._id) {
      setOpen(true)
      setSnackColor('bg-green-500')
      setSnackMessage('Team Updated successfully')
  
      setTimeout(() => {
        setOpen(false)
        setSnackColor('')
        setSnackMessage('')
        router.replace(`/teams/${res._id}`);
      }, 2500)
    }
    else{
      setDisabled(false)
  ...
  function checkIncludes(_id: string | undefined) {
    if(!_id) return

    return team?.users.includes(_id)
  }
                    ...
                    <div className='border border-gray-300 rounded-md p-4 custom-box-shadow' key={user._id}>
                      <div className='flex flex-row gap-5 flex-nowrap'>
                        ...
                        <div className='flex items-center  ml-auto'>
                          <input type="checkbox" onClick={() => addOrRemove(user._id)}
                            defaultChecked={checkIncludes(user._id)}
                          ></input>
                        </div>
                      </div>
                    </div>
              ...
              <div className="p-6">
                <div className="w-full">
                  <div className="flex flex-row-reverse w-full items-center gap-5">
                    <button className="py-2 px-5 bg-blue-500 rounded-md text-white font-semibold" onClick={() => editTeam()} disabled={disabled}>Save Team</button>
                    <Link href={`/teams/${id}`} >
