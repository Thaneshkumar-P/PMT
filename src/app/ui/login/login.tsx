  ...
  return (
    <>
      <div className={`flex flex-col gap-5 w-full justify-center items-center`} >
        <div className={`rounded-md flex flex-row items-center justify-center gap-5 border border-[#ff4545] bg-[#fa606037] p-2 w-full  ${show ? '' : 'hidden'}`}>
          <Info className="opacity-100" color="red"/>
          <h4 className="text-red-500">{error}</h4>
        </div>
        <button className={`bg-[#060606] text-white ${loading ? "w-fit rounded-full" : "w-full rounded" } p-2 transition-all`} onClick={handleLogin}>
          {loading ?
            <div id='loader' className="mt-[-6px]">
